    ...
    <div className="min-h-screen bg-[#FAF6EE]">
      ...
      <div className="bg-gradient-to-b from-[#1C0A0A] to-[#3D0F0F] px-4 py-6">
        <span className="text-[8px] bg-[#C4963A]/20 text-[#C4963A] border border-[#C4963A]/30
                         px-2 py-1 rounded-full">WinePass for Business</span>
        <h1 className="font-serif text-[18px] font-bold text-white mt-3 mb-2">
          Bordeaux Wine Experiences for Your Team
        </h1>
        <p className="text-[10px] text-[#EDE4CF] leading-relaxed mb-4">
          Custom wine events for 5–200 people. Fully managed. Invoice billing.
        ...
        <div className="flex gap-2">
          {[
            { v: '5–200', l: 'people' }, { v: '24h', l: 'response' },
            { v: 'B2B', l: 'invoice' },  { v: '100%', l: 'custom' },
          ].map(b => (
      ...
      <div className="px-4 py-4">
        <p className="text-[9px] font-medium text-[#8B6B6B] uppercase tracking-wider mb-1">
          კ-ი სახ. *
        </p>
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          <div>
            ...
            <select value={form.eventType} onChange={e => update('eventType', e.target.value)}
              className="w-full bg-white border border-[#DDD0B3] rounded-lg px-2 py-2.5
                         text-[11px] text-[#1C0A0A] outline-none">
          ...
          <div>
            ...
            <select value={form.groupSize} onChange={e => update('groupSize', e.target.value)}
              className="w-full bg-white border border-[#DDD0B3] rounded-lg px-2 py-2.5
                         text-[11px] text-[#1C0A0A] outline-none">
              <option value="5-15">5–15 კაცი</option>
              <option value="15-30">15–30 კაცი</option>
              <option value="30-50">30–50 კაცი</option>
              <option value="50+">50+ კაცი</option>
            </select>
          </div>
        </div>
        ...
        <textarea value={form.specialRequests} onChange={e => update('specialRequests', e.target.value)}
          placeholder="Languages, dietary, venue preferences..."
          rows={3}
          className="w-full bg-white border border-[#DDD0B3] rounded-lg px-3 py-2
                     text-[11px] text-[#1C0A0A] mb-4 outline-none resize-none" />
        ...
        <button onClick={handleSubmit} disabled={!ready || loading}
          ...
          {loading ? 'Sending...' : 'შ-ი გ-ა → 24h response ←'}
        </button>
        <p className="text-center text-[9px] text-[#8B6B6B] mt-2">
          ✓ No commitment · Free quote · 24h guaranteed
        </p>
      </div>
    </div>
  )
