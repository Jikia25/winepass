...
function getBundleIncludes(budget: number) {
  return {
    tasting:   true,
    transport: budget >= 40,
    lunch:     budget >= 70,
  ...
  return (
    <div className="min-h-screen bg-[#FAF6EE]">
      <div className="bg-[#5C1A1A] px-4 pt-4 pb-5">
        ...
        <h2 className="font-serif text-[15px] font-medium text-white mt-4">{b.step2Title}</h2>
        <p className="text-[10px] text-[#EDE4CF] mt-1">{b.step2Sub}</p>
      </div>

      <div className="px-4 py-4">
        <div className="text-center mb-5">
          <p className="font-serif text-[44px] font-bold text-[#5C1A1A] leading-none">€{budget}</p>
          <p className="text-[11px] text-[#8B6B6B] mt-1">{b.perPerson}</p>
          <p className="text-[10px] text-[#C4963A] mt-1.5 min-h-[16px]">✓ {inc.tagline}</p>
        </div>
          ...
          {PRESETS.map(p => (
            <button key={p} onClick={() => setBudget(p)}
              className={`text-[10px] px-3 py-1.5 rounded-full border transition-all
                ${budget===p ? 'bg-[#5C1A1A] text-white border-[#5C1A1A]' : 'bg-white text-[#5C1A1A] border-[#DDD0B3] hover:border-[#5C1A1A]'}`}>
              €{p}{p===250?'+':''}
            </button>
          ))}
        </div>
        ...
        <div className="mb-5">
          <input type="range" min={30} max={250} step={10} value={budget}
            onChange={e => setBudget(Number(e.target.value))} className="w-full accent-[#5C1A1A]" />
        ...
        <div className="bg-white border border-[#DDD0B3] rounded-lg px-3 py-2.5 mb-5">
          ...
          {[
            ...
            <div key={row.label} className="flex items-center gap-2 py-1.5 border-b border-[#EDE4CF] last:border-b-0 text-[10px]">
              ...
              <span className={`font-medium ${row.val==='✓'?'text-[#2E6B3E]':'text-[#8B6B6B]'}`}>{row.val}</span>
            </div>
          ))}
        </div>
        ...
        <p className="text-center text-[9px] text-[#8B6B6B] mt-2">{b.step2Note}</p>
      </div>
    </div>
  )
}
