    ...
    <div className="min-h-screen bg-[#FAF6EE]">
      <div className="bg-gradient-to-b from-[#3D0F0F] to-[#5C1A1A] px-4 py-6">
        ...
        <div className="flex gap-2">
          {[{v:'0',l:p.bookings},{v:'0',l:p.stamps},{v:'0',l:p.reviews}].map(s => (
      ...
      <div className="px-4 py-4">
        <div className="bg-[#3D0F0F] rounded-xl px-4 py-3 mb-4">
          <div className="flex justify-between items-center mb-3">
            <p className="font-serif text-[13px] text-white font-medium">{p.passport}</p>
        ...
        <div className="bg-white border border-[#DDD0B3] rounded-xl p-4 mb-3">
          <div className="flex justify-between items-center mb-3">
            <p className="text-[12px] font-medium text-[#1C0A0A]">{p.myBookings}</p>
            <Link href="/build" className="text-[10px] text-[#5C1A1A]">{p.newBooking}</Link>
          </div>
          <div className="text-center py-4">
            <p className="text-2xl mb-2">🍷</p>
            <p className="text-[12px] text-[#5C1A1A] font-serif font-medium">{p.noBookings}</p>
            <p className="text-[10px] text-[#8B6B6B] mt-1 mb-3">{p.noBookingsSub}</p>
        ...
        <div className="bg-white border border-[#DDD0B3] rounded-xl overflow-hidden mb-3">
          {[
            {icon:'🎁', label:p.myGifts,       href:'/gift'},
            {icon:'⭐', label:p.myReviews,      href:'/profile/reviews'},
            {icon:'❤️', label:p.savedChateaux,  href:'/search'},
            {icon:'🔔', label:p.notifications,  href:'/profile'},
          ].map((item,i,arr) => (
