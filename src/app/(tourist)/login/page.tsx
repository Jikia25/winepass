...
export default function LoginPage() {
  ...
  async function handleLogin() {
    ...
    if (e) { setError(l.wrongCredentials); setLoading(false) }
  ...
  async function handleRegister() {
    ...
    if (e) {
      if (e.message.toLowerCase().includes('already')||e.message.toLowerCase().includes('registered')) {
        setError(l.emailAlreadyExists); setTimeout(()=>{ setEmail(email); rst('forgot') },1800)
      } else setError(e.message)
    } else setSuccess(l.emailSent(email))
  }
  ...
  async function handleForgot() {
    ...
    else { setSuccess(l.codeSent(email)); setTimeout(()=>rst('verify-otp'),1500) }
  }
  ...
  async function handleVerifyOtp() {
    ...
    setLoading(false)
    if (e) setError('Code incorrect or expired.')
    else rst('new-password')
  }

  async function handleNewPassword() {
    if (!pwOk||password!==password2) { setError(l.passwordsMismatch); return }
    setLoading(true); setError('')
    const {error:e} = await supabase.auth.updateUser({password})
    setLoading(false)
    if (e) setError(e.message)
    else { setSuccess(l.passwordUpdated); setTimeout(()=>{ setPass(''); setPass2(''); rst('login') },2000) }
  }

  function PwHint() {
    const v = { len: password.length>=8, upper: /[A-Z]/.test(password), lower: /[a-z]/.test(password), digit: /[0-9]/.test(password), special: /[^A-Za-z0-9]/.test(password) }
    const items: [keyof typeof v, string][] = [['len',l.pw8],['upper',l.pwUpper],['lower',l.pwLower],['digit',l.pwDigit],['special',l.pwSpecial]]
    return (
      <div className="grid grid-cols-2 gap-1 mb-3">
        {items.map(([k,label]) => (
          <span key={k} className={`text-[9px] flex items-center gap-1 ${v[k]?'text-green-600':'text-[#8B6B6B]'}`}>{v[k]?'✓':'○'} {label}</span>
        ))}
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-[#FAF6EE] flex items-center justify-center px-4">
      <div className="max-w-sm w-full">
        <div className="text-center mb-6">
          ...
          <h1 className="font-serif text-[20px] font-bold text-[#1C0A0A]">{l.title}</h1>
          <p className="text-[11px] text-[#8B6B6B] mt-1">{l.subtitle}</p>
          {redirectTo!=='/' && <p className="text-[10px] text-[#C4963A] mt-2 bg-[#FFF8EC] px-3 py-1.5 rounded-lg">{l.bookingPrompt}</p>}
        </div>

        {stage==='forgot' && (
          <div className="bg-white border border-[#DDD0B3] rounded-xl p-5">
            <button onClick={()=>rst('login')} className="text-[10px] text-[#5C1A1A] mb-4 flex items-center gap-1">{l.backToLogin}</button>
            <h2 className="font-serif text-[15px] font-medium text-[#1C0A0A] mb-1">{l.forgotTitle}</h2>
            <p className="text-[11px] text-[#8B6B6B] mb-4">{l.forgotSub}</p>
        ...
        {stage==='verify-otp' && (
          <div className="bg-white border border-[#DDD0B3] rounded-xl p-5">
            <button onClick={()=>rst('forgot')} className="text-[10px] text-[#5C1A1A] mb-4 flex items-center gap-1">← Back</button>
            <h2 className="font-serif text-[15px] font-medium text-[#1C0A0A] mb-1">{l.verifyTitle}</h2>
            <p className="text-[11px] text-[#8B6B6B] mb-4">{l.verifySub} {email}</p>
            ...
            <button onClick={handleVerifyOtp} disabled={loading||otp.length<6}
              ...
              {loading?l.verifying:l.verifyBtn}
            </button>
            <button onClick={handleForgot} className="w-full text-center text-[10px] text-[#5C1A1A] mt-3 hover:underline">{l.resendCode}</button>
          </div>
        )}

        {stage==='new-password' && (
          <div className="bg-white border border-[#DDD0B3] rounded-xl p-5">
            <h2 className="font-serif text-[15px] font-medium text-[#1C0A0A] mb-1">{l.newPasswordTitle}</h2>
            <p className="text-[11px] text-[#8B6B6B] mb-4">{l.newPasswordSub}</p>
            <label className="block text-[9px] font-medium text-[#8B6B6B] uppercase tracking-wider mb-1">{l.newPassLabel}</label>
            <PassInput val={password} set={setPass} show={showPass} toggle={()=>setShowPass(!showPass)} />
            {password && <PwHint />}
            <label className="block text-[9px] font-medium text-[#8B6B6B] uppercase tracking-wider mb-1">{l.confirmPassLabel}</label>
            <PassInput val={password2} set={setPass2} show={showPass2} toggle={()=>setShowPass2(!showPass2)} mb="mb-4" />
            {password2&&password!==password2 && <p className="text-[10px] text-[#A32D2D] mb-2">{l.passwordsMismatch}</p>}
            {error && <p className="text-[11px] text-[#A32D2D] bg-[#FCEBEB] rounded-lg px-3 py-2 mb-3">{error}</p>}
            {success && <p className="text-[11px] text-[#2E6B3E] bg-[#EAF3DE] rounded-lg px-3 py-2 mb-3">{success}</p>}
            <button onClick={handleNewPassword} disabled={loading||!pwOk||password!==password2}
              ...
              {loading?l.saving:l.updatePassword}
            </button>
          </div>
        )}
        ...
        {(stage==='login'||stage==='register') && (
          <>
            <div className="flex bg-white border border-[#DDD0B3] rounded-xl p-1 mb-4">
              <button onClick={()=>rst('login')} className={`flex-1 py-2 rounded-lg text-[12px] font-medium transition-all ${stage==='login'?'bg-[#5C1A1A] text-white':'text-[#8B6B6B]'}`}>{l.signIn}</button>
              <button onClick={()=>rst('register')} className={`flex-1 py-2 rounded-lg text-[12px] font-medium transition-all ${stage==='register'?'bg-[#5C1A1A] text-white':'text-[#8B6B6B]'}`}>{l.register}</button>
            </div>
            <div className="bg-white border border-[#DDD0B3] rounded-xl p-5">
              <button onClick={signInWithGoogle} disabled={loading}
                ...
                {stage==='login'?l.googleSignIn:l.googleRegister}
              </button>
              <div className="flex items-center gap-2 mb-4">
                <div className="flex-1 h-px bg-[#EDE4CF]"/>
                <span className="text-[10px] text-[#8B6B6B]">{l.orEmail}</span>
                <div className="flex-1 h-px bg-[#EDE4CF]"/>
              </div>
              {stage==='register' && (
                <>
                  <label className="block text-[9px] font-medium text-[#8B6B6B] uppercase tracking-wider mb-1">{l.nameLabel}</label>
              ...
              <label className="block text-[9px] font-medium text-[#8B6B6B] uppercase tracking-wider mb-1">{l.emailLabel}</label>
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="your@email.com"
                className="w-full border border-[#DDD0B3] rounded-lg px-3 py-2.5 text-[12px] mb-3 outline-none focus:border-[#5C1A1A]" />
              <div className="flex justify-between items-center mb-1">
                <label className="text-[9px] font-medium text-[#8B6B6B] uppercase tracking-wider">{l.passwordLabel}</label>
                {stage==='login' && <button onClick={()=>rst('forgot')} className="text-[9px] text-[#5C1A1A] hover:underline">{l.forgotPassword}</button>}
              </div>
              ...
              <button onClick={stage==='login'?handleLogin:handleRegister}
                ...
                {loading?(stage==='login'?l.signingIn:l.registering):(stage==='login'?l.signInBtn:l.registerBtn)}
              </button>
            </div>
          </>
        )}
        <p className="text-center text-[10px] text-[#8B6B6B] mt-3">{l.terms}</p>
      </div>
    </div>
  )
}
