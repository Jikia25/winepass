...
export default function BookingPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data }) => {
      if (!data.user) {
        sessionStorage.setItem('redirect_after_login', `/book/${slug}`)
        router.push('/login')
      } else {
    ...
    setMounted(true)
    const b = parseInt(sessionStorage.getItem('wp_budget') ?? '80')
    const p = parseInt(sessionStorage.getItem('wp_count')  ?? '2')
    setBudget(b)
    setPersons(p)
  }, [])
  ...
  async function handlePay() {
    ...
    setError('')
    try {
      router.push(`/pass/WP-DEMO-${Date.now()}`)
    } catch {
      setError('გადახდა ვერ მოხდა')
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#FAF6EE]">
      <div className="bg-[#3D0F0F] px-4 py-3">
        <p className="font-serif text-[13px] text-white font-medium capitalize">
          {slug.replace(/-/g,' ')} — Classic Bundle
        </p>
        <p className="text-[9px] text-[#EDE4CF] mt-0.5">
          €{budget}/კაცი · {persons} კაცი · სულ: €{subtotal}
        </p>
      </div>

      <div className="px-4 py-4">
        <div className="bg-[#3D0F0F] rounded-lg px-3 py-2.5 mb-4">
          <p className="text-[9px] text-[#C4963A] uppercase tracking-wider font-medium mb-2">Bundle — Classic</p>
        ...
        <p className="text-[9px] font-medium text-[#8B6B6B] uppercase tracking-wider mb-2">თარიღი</p>
        <div className="grid grid-cols-4 gap-2 mb-4">
          {dates.map(d => (
            <button key={d.date} onClick={() => setDate(d.date)}
              className={`rounded-lg py-2 px-1 text-center border transition-all
                ${date===d.date ? 'border-[#5C1A1A] bg-[#FAF0F0]' : 'bg-white border-[#DDD0B3]'}`}>
        ...
        <p className="text-[9px] font-medium text-[#8B6B6B] uppercase tracking-wider mb-2">დრო</p>
        <div className="flex gap-2 mb-4">
          {TIME_SLOTS.map(t => (
            <button key={t} onClick={() => setTime(t)}
              className={`flex-1 py-2.5 rounded-lg text-[11px] border transition-all
                ${time===t ? 'bg-[#5C1A1A] text-white border-[#5C1A1A]' : 'bg-white text-[#5C1A1A] border-[#DDD0B3]'}`}>
              {t}
            </button>
          ))}
        </div>

        <p className="text-[9px] font-medium text-[#8B6B6B] uppercase tracking-wider mb-2">ელ-ფოსტა</p>
        <input type="email" value={email} onChange={e => setEmail(e.target.value)}
          placeholder="your@email.com"
          className="w-full bg-white border border-[#DDD0B3] rounded-lg px-3 py-2.5
                     text-[12px] mb-4 outline-none focus:border-[#5C1A1A]" />

        <div className="bg-white border border-[#DDD0B3] rounded-lg px-3 py-2 mb-4">
          {[
            {l:`€${budget} × ${persons} კაცი`, v:`€${subtotal}`},
            {l:'TVA 10%', v:`€${tax}`},
            {l:'Booking fee', v:'€0', green:true},
          ].map(r => (
          ...
          <div className="flex justify-between pt-2 text-[13px] font-semibold">
            <span className="text-[#8B6B6B]">სულ</span>
        ...
        <button onClick={handlePay} disabled={!canBook || loading}
          className={`w-full py-3 rounded-[7px] text-[12px] font-medium font-serif transition-all mb-2
            ${canBook && !loading ? 'bg-[#5C1A1A] text-white' : 'bg-[#5C1A1A]/30 text-white/50 cursor-not-allowed'}`}>
          {loading ? 'Processing...' : `გადახდა €${total} →`}
        </button>
        <button onClick={handlePay} disabled={!canBook || loading}
          className={`w-full py-2.5 rounded-[7px] text-[12px] flex items-center justify-center gap-1.5 mb-2
            ${canBook ? 'bg-black text-white' : 'bg-black/30 text-white/50 cursor-not-allowed'}`}>
          <span className="text-[14px]">&#63743;</span> Apple Pay · €{total}
        </button>
        <p className="text-center text-[8px] text-[#8B6B6B] mt-2">
          Free cancellation up to 48h · Login required
        </p>
      </div>
    </div>
  )
