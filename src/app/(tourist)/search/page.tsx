...
export default function SearchPage() {
  ...
  const suggestions = q.length >= 2 ? APPELLATIONS.filter(a => a.name.toLowerCase().includes(q.toLowerCase())) : []
  ...
  return (
    <div className="min-h-screen bg-[#FAF6EE]">
      <div className="bg-[#5C1A1A] px-4 py-4">
        <p className="text-[9px] text-[#EDE4CF] mb-2">Where do you want wine?</p>
          ...
          <div className="flex-1 flex items-center gap-2 bg-white rounded-lg px-3 py-2.5">
            ...
            {q && (
              <button onClick={() => { setQ(''); setShowSugg(false) }}
                className="text-[9px] text-[#5C1A1A] bg-[#EDE4CF] px-1.5 py-0.5 rounded">✕</button>
            )}
          </div>
          <button onClick={() => handleSearch()}
            className="bg-[#C4963A] text-[#3D0F0F] px-3 py-2 rounded-lg text-[11px] font-medium font-serif">→</button>
        </div>
        {!q && (
          <>
            <p className="text-[9px] text-[#EDE4CF]/70 mt-3 mb-2">{s.popularLabel}</p>
            <div className="flex gap-2 flex-wrap">
              {POPULAR_SEARCHES.map(ps => (
                <button key={ps} onClick={() => { setQ(ps); handleSearch(ps) }}
                  className="text-[10px] bg-white/12 text-[#EDE4CF] px-2.5 py-1 rounded-full border border-white/20 hover:bg-[#C4963A]/25 hover:text-[#C4963A] transition">
                  {ps}
                </button>
              ))}
            </div>
      ...
      {showSugg && suggestions.length > 0 && (
        <div className="bg-white shadow-sm">
          {suggestions.map(a => (
            <button key={a.slug} onClick={() => router.push(`/regions/${a.slug}`)}
              className="w-full flex items-center gap-3 px-4 py-3 border-b border-[#EDE4CF] last:border-b-0 hover:bg-[#FAF6EE] transition text-left">
              <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: `#${a.color}` }} />
              <span className="flex-1 text-[12px] text-[#1C0A0A]">{a.name}</span>
              <span className="text-[9px] text-[#8B6B6B]">{a.count}</span>
      ...
      {!q && (
        <div className="px-4 py-4">
          <p className="text-[9px] font-medium text-[#8B6B6B] uppercase tracking-wider mb-3">{s.browseLabel}</p>
          <div className="flex flex-col gap-2">
            {APPELLATIONS.map(a => (
              <button key={a.slug} onClick={() => router.push(`/regions/${a.slug}`)}
                className="flex items-center gap-3 bg-white border border-[#DDD0B3] rounded-lg px-3 py-2.5 hover:border-[#5C1A1A] transition text-left">
